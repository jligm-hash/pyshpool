#!/bin/bash
# each node one task but multiple thread
...
for i in $(seq 1 $nNode)
do
    # echo "srun --nodes=1 --ntasks=1 --cpus-per-task=$nCpu ./poolRunShell-thread.sh -c $nCpu -i inpDL${i}.out > logNode${i}.out &" >> tmp.git

        echo "srun --nodes=1 --ntasks=1 --cpus-per-task=$nCpu ./poolRunShell-static -c 1 -i inpDL${i}.out > logNode${i}.out &" >> tmp.git
